...
//
// THE RE-ASK. An adversary that returns no valid nonce-signed verdict line gets
// re-asked once (callAdversary in orchestrate.mjs), and readiness does the same.
// That is a second full billed call, and it is common rather than exceptional —
// in the first real run of this estimator, one adversary re-asked in all three
// rounds, so 12 calls happened where a one-call-per-round model predicted 9.
// The ceiling therefore prices EVERY adversary call as two. A run where nobody
// re-asks comes in at roughly half the adversary share, which is the direction
// an honest ceiling should err in.
//
// What is NOT modelled: `retries`, which fire on transport failure (timeout,
// 5xx). A call that fails that way returns no usage and so bills nothing on
// OpenRouter — a retried request is a repeat of an unbilled one, not an extra
// charge. If a provider ever bills partial generations on timeout this would
// become an undercount, which is why it is written down rather than assumed away.
...
// An adversary is called once, then re-asked once if its verdict line is missing
// or carries the wrong session token. Two is the worst case and the number the
// ceiling prices; see the header note on THE RE-ASK.
export const ADVERSARY_CALLS_PER_ROUND = 2;
...
export function estimateRun({ taskChars, rounds, mode, proposer, adversaries = [], priceOf }) {
  ...
  // readiness is one gate call — but it re-asks once if the verdict line is
  // missing or unsigned, exactly like an adversary, so the ceiling prices two.
  if (mode === "readiness") {
    const inTok = taskTok + sysTok(proposer);
    const reaskIn = inTok + cap(proposer);      // the re-ask feeds the first reply back
    rows.push({ name: proposer.name, model: proposer.model, calls: 2,
      ...agentCost(proposer, inTok + reaskIn, cap(proposer) * 2, priceOf) });
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    adversaries.forEach((a, i) => {
      ...
      const firstIn = sysTok(a) + taskTok + propCap + (round > 1 ? toTokens(PREV_REVIEW_CAP) : 0);
      // The re-ask repeats that prompt and appends the first reply, so its input
      // is the first call's input plus one full adversary output.
      advIn[i] += firstIn + (firstIn + cap(a));
    });
    advCalls += adversaries.length * ADVERSARY_CALLS_PER_ROUND;
  }

  rows.push({ name: proposer.name, model: proposer.model, calls: propCalls,
    ...agentCost(proposer, propIn, propCap * propCalls, priceOf) });
  adversaries.forEach((a, i) => {
    const calls = rounds * ADVERSARY_CALLS_PER_ROUND;
    rows.push({ name: a.name, model: a.model, calls,
      ...agentCost(a, advIn[i], cap(a) * calls, priceOf) });
  });
